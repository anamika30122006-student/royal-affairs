...
function renderSuggestions(dropdown, query) {
  ...
  searchMatches.slice(0, 5).forEach(prod => {
    ...
    const linkUrl = prod.slug ? `product.html?slug=${encodeURIComponent(prod.slug)}` : `product.html?id=${encodeURIComponent(prod.id)}`;

    const prodImg = typeof resolveProductImage === "function" ? resolveProductImage(prod, 0) : (prod.images && prod.images[0] ? prod.images[0] : "./assets/images/anarkali_maroon.jpg");

    html += `
      <a href="${linkUrl}" class="search-result-row" onclick="saveRecentSearch('${prod.name}')" style="text-decoration: none; display: flex; align-items: center; padding: 0.75rem 1.25rem; border-bottom: 1px solid var(--color-gray-light); border-left: 3px solid transparent; transition: all var(--transition-fast);">
        <img src="${prodImg}" alt="${prod.name}" style="width: 40px; height: 52px; object-fit: cover; border-radius: var(--border-radius-xs); margin-right: 1rem; border: 1px solid var(--color-gray-light);" onerror="handleImageError(this)">
        <div style="flex: 1;">
