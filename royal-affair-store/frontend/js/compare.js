...
function renderComparePage() {
  ...
  fullProducts.forEach(prod => {
    const compLink = prod.slug ? `product.html?slug=${encodeURIComponent(prod.slug)}` : `product.html?id=${encodeURIComponent(prod.id)}`;
    headerRow += `<th style="text-align: center; font-family: var(--font-heading); font-size: 1.15rem; color: var(--color-maroon-dark);"><a href="${compLink}" style="text-decoration: none; color: inherit;">${prod.name}</a></th>`;
    imageRow += `
      <td style="text-align: center; padding: 1.25rem;">
        <a href="${compLink}"><img src="${resolveProductImage(prod, 0)}" alt="${prod.name}" style="width: 100px; height: 135px; object-fit: cover; margin: 0 auto; box-shadow: var(--shadow-sm); border-radius: var(--border-radius-xs); border: 1px solid var(--color-gray-light);" onerror="handleImageError(this)"></a>
      </td>
    `;
