  ...
  wishlist.forEach(item => {
    ...
    const availHtml = originalProd.stock > 0
      ...
      : `<span style="color: var(--color-error); font-weight: 600; font-size: 0.85rem; display: inline-flex; align-items: center; gap: 4px;"><span style="width: 6px; height: 6px; border-radius: 50%; background-color: var(--color-error); display: inline-block;"></span>Out of Stock</span>`;
            ...
            const targetUrl = originalProd.slug ? `product.html?slug=${encodeURIComponent(originalProd.slug)}` : `product.html?id=${encodeURIComponent(originalProd.id)}`;
            html += `
              <tr style="border-bottom: 1px solid var(--color-gray-light);">
                <td style="display: flex; align-items: gap: 1.25rem; padding: 1.5rem 0; text-align: left;">
                  <a href="${targetUrl}"><img src="${resolveProductImage(originalProd, 0)}" alt="${originalProd.name}" style="width: 70px; height: 95px; object-fit: cover; border-radius: var(--border-radius-xs); border: 1px solid var(--color-gray-light);" onerror="handleImageError(this)"></a>
                  <div>
                    <h3 style="font-family: var(--font-heading); font-size: 1.15rem; color: var(--color-maroon-dark); margin: 0 0 0.5rem 0; line-height: 1.2;">
                      <a href="${targetUrl}" style="text-decoration: none; color: inherit;">${originalProd.name}</a>
                    </h3>
