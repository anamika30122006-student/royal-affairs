...
function initCatalogPage() {
  ...
  if (urlParams.has("collection")) {
    const colVal = urlParams.get("collection");
    catalogState.search = colVal.toLowerCase();
    const infoMsg = document.getElementById("search-info-message");
    if (infoMsg) {
      infoMsg.textContent = `Showing collection: "${colVal}"`;
...
async function applyFilters() {
  ...
  if (grid && typeof getLoadingSkeletonHTML === 'function') {
    grid.innerHTML = getLoadingSkeletonHTML(6);
  }

  // Construct query params matching API requirements
  const params = {
    page: catalogState.currentPage,
    limit: catalogState.itemsPerPage
  };

  if (catalogState.search) params.search = catalogState.search;
  if (catalogState.category && catalogState.category !== "All") params.category = catalogState.category;
  if (catalogState.maxPrice) params.max_price = catalogState.maxPrice;
  if (catalogState.sizes && catalogState.sizes.length > 0) params.size = catalogState.sizes[0];
  if (catalogState.colors && catalogState.colors.length > 0) params.color = catalogState.colors[0];
  if (catalogState.fabrics && catalogState.fabrics.length > 0) params.fabric = catalogState.fabrics[0];
  if (catalogState.occasions && catalogState.occasions.length > 0) params.occasion = catalogState.occasions[0];
  if (catalogState.availability === "in-stock") params.in_stock = true;

  if (catalogState.sort === "price-low") params.sort = "price_asc";
  else if (catalogState.sort === "price-high") params.sort = "price_desc";
  else if (catalogState.sort === "rating") params.sort = "rating";
  else if (catalogState.sort === "discount") params.sort = "discount";
  else if (catalogState.sort === "newest") params.sort = "newest";

  const apiRes = await getProducts(params);
  const items = apiRes.products || [];

  window.currentFilteredProducts = items;

  renderFilterChips();

  if (countLabel) {
    countLabel.textContent = `${apiRes.total || items.length} ${(apiRes.total || items.length) === 1 ? 'suit' : 'suits'} found`;
  }

  if (items.length === 0) {
    if (grid) grid.innerHTML = typeof getEmptyStateHTML === 'function' ? getEmptyStateHTML("Adjust your filter selections to see our luxury pieces.") : '';
  ...
  if (grid) {
    grid.innerHTML = items.map(prod => createProductCardHTML(prod)).join("");
  }

  // Sync Wishlist & Compare button states
  ...
  if (typeof renderPaginationRow === "function") {
    renderPaginationRow(apiRes.page || 1, apiRes.totalPages || 1);
  }
}
