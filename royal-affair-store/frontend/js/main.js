...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Load dynamic Shop By Style categories & Curated Collections
  loadShopByStyleCategories();
      ...
      <div class="navigation-row">
        <div class="container" style="position: relative; display: flex; align-items: center; justify-content: space-between; width: 100%;">
          ...
          <div class="nav-item-dropdown category-dropdown-trigger" style="position: relative; flex-shrink: 0;">
            <button class="category-dropdown-btn" aria-haspopup="true" aria-expanded="false" type="button">
...
function initGlobalHeaderScroll() {
  ...
  const updateHeaderState = () => {
    ...
    // Separate enter/exit thresholds prevent the header height change from
    // repeatedly crossing one boundary and causing visible scroll jitter.
    if (!isSticky && scrollY > 120) {
      header.classList.add("sticky");
      isSticky = true;
    } else if (isSticky && scrollY < 20) {
...
function initDrawerAndOverlays() {
  ...
  document.addEventListener("click", (e) => {
    ...
    // Toggle Category Dropdown Menu on Click
    const catTrigger = e.target.closest(".category-dropdown-trigger");
    if (catTrigger) {
      const btn = catTrigger.querySelector("button, .nav-link");
      if (btn && (btn.contains(e.target) || btn === e.target)) {
        e.preventDefault();
        e.stopPropagation();
    ...
    if (!e.target.closest(".category-dropdown-trigger")) {
      document.querySelectorAll(".category-dropdown-trigger.open").forEach(el => {
...
function createProductCardHTML(prod) {
  ...
  const mainImg = resolveProductImage(prod, 0);
  const hoverImg = resolveProductImage(prod, 1);

  return `
    <div class="product-card fade-in" style="${isSoldOut ? 'opacity: 0.85;' : ''}">
      <div class="product-img-wrapper">
        <a href="product.html?id=${prod.id}">
          <img src="${mainImg}" alt="${prod.name}" class="product-img" onerror="handleImageError(this)">
          <img src="${hoverImg}" alt="${prod.name} back" class="product-img-hover" onerror="handleImageError(this)">
        </a>
      ...
      <div class="product-content">
        ...
        <div style="margin-top: 8px;">
          <div class="product-rating">
            <span class="rating-stars">Ôÿà ${prod.rating}</span>
    ...
    <div class="modal-overlay open" id="quick-view-modal-overlay">
      <div class="modal-container" style="max-width: 750px; display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: var(--spacing-lg); padding: var(--spacing-lg);">
        ...
        <div style="aspect-ratio: 3/4; overflow: hidden; border-radius: var(--border-radius-xs);">
          <img src="${resolveProductImage(prod, 0)}" alt="${prod.name}" style="width: 100%; height: 100%; object-fit: cover;" onerror="handleImageError(this)">
        </div>
        ...
        <div style="display: flex; flex-direction: column; justify-content: space-between; text-align: left;">
          <div>
            ...
            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: var(--spacing-sm);">
              <span style="color: var(--color-gold); font-size: var(--font-sm);">Ôÿà ${prod.rating}</span>
...
function injectSEO() {
  ...
  if (pathname.includes("product.html") && typeof getProductById === "function") {
    ...
    if (product) {
      document.title = `${product.name} | Royal Affair ÔÇô Designer Suits`;
      if (metaDescEl) {
...
// 17. Load Dynamic "Shop By Style" Category Cards
async function loadShopByStyleCategories() {
  const container = document.getElementById("shop-by-style-container") || document.querySelector(".categories-grid");
  if (!container) return;
  ...
  // 1. Fetch active categories from backend (support multiple endpoints and response payload formats)
  const endpoints = [
    `${apiHost}/api/v1/categories`,
    `${apiHost}/api/categories`,
    `/api/v1/categories`,
    `/api/categories`
  ];
    ...
    try {
      ...
      if (res.ok) {
        ...
        const extracted = Array.isArray(rawData)
          ? rawData
          : (rawData.categories || rawData.data || rawData.items || rawData.result || []);
      ...
      console.warn(`Category fetch attempt failed for ${endpoint}:`, err);
    }
  }

  // 2. Fetch products for image fallback
  try {
    if (typeof getAllProductsAsync === "function") {
      productsList = await getAllProductsAsync();
    } else {
      const pRes = await fetch(`${apiHost}/api/v1/products`);
      if (pRes.ok) {
        const pRaw = await pRes.json();
        productsList = Array.isArray(pRaw) ? pRaw : (pRaw.products || pRaw.data || []);
      }
    }
  } catch (e) {
    console.warn("Product fetch for category image fallback failed:", e);
  }

  if ((!productsList || productsList.length === 0) && typeof products !== "undefined") {
    productsList = products;
  }

  // Hide disabled or deleted categories (check is_active, active, published, status)
  categories = (categories || []).filter(cat => {
    if (!cat) return false;
    if (cat.is_active === false || cat.active === false || cat.published === false) return false;
    if (cat.status === "disabled" || cat.status === "inactive" || cat.status === "archived") return false;
  ...
  // Admin-selected seasonal/festival cards come first. Fill any remaining
  // slots from active categories, and never render more than four cards.
  const homeSelected = categories
    .filter(cat => cat.show_on_home === true)
    .sort((a, b) => (a.home_display_order || 99) - (b.home_display_order || 99));
  const selectedIds = new Set(homeSelected.map(cat => cat.id || cat._id || cat.slug || cat.name));
  const remaining = categories.filter(cat => !selectedIds.has(cat.id || cat._id || cat.slug || cat.name));
  categories = [...homeSelected, ...remaining].slice(0, 4);

  // Fallback to default list if backend returned empty array
  if (categories.length === 0) {
    categories = [
      { name: "Anarkali Suits", slug: "anarkali", image: null },
      { name: "Sharara Sets", slug: "sharara-set", image: null },
      { name: "Salwar Kameez", slug: "salwar-kameez", image: null },
      { name: "Palazzo Suits", slug: "palazzo-suit", image: null }
    ];
  }

  let cardsHtml = "";
  categories.forEach(cat => {
    const catName = cat.name || cat.title || cat.category_name || "Category";
    const catSlug = cat.slug || cat.category_slug || catName.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-");
    const linkUrl = `shop.html?category=${encodeURIComponent(catName)}`;

    // Image priority:
    // 1. Admin-uploaded Category Cover Image
    // 2. First featured product image from that category
    // 3. First available product image from that category
    // 4. Default fallback placeholder
    let coverImage = cat.image || cat.cover_image || cat.image_url || cat.imageUrl || cat.category_image || cat.thumbnail || cat.cover || null;
    ...
    // Fallback 1: First featured product image
    if (!coverImage || coverImage.trim() === "" || coverImage.includes("placeholder")) {
      const featuredProd = matchingProducts.find(p => p.featured || p.is_featured);
      if (featuredProd) {
        coverImage = resolveProductImage(featuredProd, 0);
      }
    }

    // Fallback 2: First available product image
    if (!coverImage || coverImage.trim() === "" || coverImage.includes("placeholder")) {
      const firstProd = matchingProducts.find(p => p);
      if (firstProd) {
        coverImage = resolveProductImage(firstProd, 0);
      }
    }

    coverImage = resolveStorefrontImg(coverImage);

    const escapedName = String(catName).replace(/"/g, "&quot;");

    cardsHtml += `
      <div class="category-card fade-in">
        <a href="${linkUrl}">
          <div class="category-card-img-wrapper">
            <img src="${coverImage}" alt="${escapedName}" class="category-card-img" loading="lazy" onerror="handleImageError(this)">
          </div>
          <div class="category-overlay">
            <h3>${escapedName}</h3>
            <span class="category-action">View Style &rarr;</span>
          </div>
        </a>
      </div>
    `;
...
// 18. Dynamic Storefront Collections Showcase — fully driven by Admin Collections API
async function loadCuratedCollections() {
  const tabNav   = document.getElementById("curated-tab-nav");
  const tabPanes = document.getElementById("curated-tab-panes");
  if (!tabNav || !tabPanes) return;

  const isLocal = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1" || window.location.protocol === "file:";
  const apiHost = isLocal ? "http://127.0.0.1:8000" : "";

  // 1. Fetch active collections list (already includes product_ids + product_count)
  let collections = [];
  try {
    const res = await fetch(`${apiHost}/api/v1/collections`);
    if (res.ok) {
      const data = await res.json();
      collections = Array.isArray(data) ? data : (data.collections || data.data || []);
    }
  } catch (err) {
    console.warn("Could not fetch storefront collections:", err);
  }

  // Filter only active collections, sorted by display_order
  ...
  if (collections.length === 0) {
    tabNav.innerHTML  = "";
    tabPanes.innerHTML = `<p style="text-align:center; color:var(--color-gray); padding:3rem;">No active collections configured yet.</p>`;
    return;
  }

  // 2. Build tab buttons
  tabNav.innerHTML = collections.map((col, idx) => {
    const tabId = `col-tab-${col.id || col.slug || idx}`;
    return `
      <button class="tab-btn curated-tab-btn ${idx === 0 ? 'active' : ''}"
        data-tab="${tabId}"
        onclick="switchCollectionTab(this, '${tabId}')">
        ${col.name}
      </button>
    `;
  }).join("");

  // 3. Fetch all 4 collections in parallel (much faster than serial)
  const detailFetches = collections.map(col =>
    fetch(`${apiHost}/api/v1/collections/${col.id || col.slug}`)
      .then(r => r.ok ? r.json() : null)
      .catch(() => null)
  );
  const detailResults = await Promise.all(detailFetches);

  // 4. Build all tab panes from the parallel results
  let panesHtml = "";
  for (let i = 0; i < collections.length; i++) {
    const col    = collections[i];
    const detail = detailResults[i];
    const tabId  = `col-tab-${col.id || col.slug || i}`;
    let cardsHtml = "";

    if (detail && Array.isArray(detail.products) && detail.products.length > 0) {
      const prods = detail.products.slice(0, 4);
      cardsHtml = prods.map(p => {
        const pid      = p.id || p._id || p.slug;
        const pSlug    = p.slug || pid;
        const pName    = p.name || "Designer Suit";
        // Use subcategory (e.g. "Pakistani Suits") or occasion as the visible label
        const pCat     = p.subcategory || p.occasion || p.brand || "Designer Suit";
        const pPrice   = parseFloat(p.price || 0);
        const pOrig    = parseFloat(p.original_price || p.originalPrice || pPrice);
        const imgSrc   = resolveProductImage(p, 0);
        const discount = (pOrig > pPrice) ? Math.round(((pOrig - pPrice) / pOrig) * 100) : 0;

        return `
          <div class="curated-product-card">
            <div class="curated-card-img-wrap">
              <a href="product.html?id=${pSlug}">
                <img src="${imgSrc}" alt="${pName}" class="curated-card-img"
                  onerror="handleImageError(this)">
              </a>
              ${discount > 0 ? `<span class="curated-discount-badge">-${discount}%</span>` : ""}
              <button class="curated-quick-add-btn" onclick="addToCart('${pid}', 'M', 1)">Quick Add +</button>
            </div>
            <div class="curated-card-info">
              <span class="curated-card-cat">${pCat}</span>
              <h3 class="curated-card-name">
                <a href="product.html?id=${pSlug}">${pName}</a>
              </h3>
              <div class="curated-card-price">
                <span class="curated-price-current">₹${pPrice.toLocaleString("en-IN")}</span>
                ${pOrig > pPrice ? `<span class="curated-price-original">₹${pOrig.toLocaleString("en-IN")}</span>` : ""}
              </div>
            </div>
          </div>
        `;
      ...
      cardsHtml = `<p class="curated-empty-msg">No suits added to this collection yet.</p>`;
    }

    const seeMoreUrl = `shop.html?collection=${encodeURIComponent(col.slug || col.name)}`;

    panesHtml += `
      <div id="${tabId}" class="tab-pane curated-tab-pane ${i === 0 ? 'active' : ''}">
        <div class="curated-collection-grid">
          ${cardsHtml}
        </div>
        <div class="curated-see-more-wrap">
          <a href="${seeMoreUrl}" class="btn btn-secondary curated-see-more-btn">
            See More ${col.name} &rarr;
          </a>
        </div>
      </div>
    `;
  ...
  tabPanes.innerHTML = panesHtml;
}

function switchCollectionTab(btn, tabId) {
  const container = btn.closest(".curated-tab-container") || btn.closest(".tab-container");
  if (!container) return;
...
// 19. Dynamically Synchronize Search Bar & Nav Dropdown Categories
async function populateHeaderCategories() {
  const searchSelect = document.querySelector(".search-category-select");
  const navDropdownMenu = document.querySelector(".category-dropdown-menu");
  const mobileSubmenu = document.querySelector(".mobile-drawer-submenu");
  ...
  let categories = [];
  try {
    const res = await fetch(`${apiHost}/api/v1/categories`);
    if (res.ok) {
      const data = await res.json();
      categories = Array.isArray(data) ? data : (data.categories || data.data || data.items || []);
    }
  } catch (err) {
    console.warn("Could not load header categories:", err);
  }

  // Filter active categories
  categories = (categories || []).filter(c => {
    if (c.is_active === false || c.active === false || c.published === false || c.status === 'disabled') return false;
    return true;
  });

  // 1. Update Search Bar Categories Dropdown Select
  if (searchSelect) {
    let selectOptions = `<option value="">All Categories</option>`;
    categories.forEach(cat => {
      const cName = cat.name || cat.title;
      selectOptions += `<option value="${cName}">${cName}</option>`;
    });
    searchSelect.innerHTML = selectOptions;

    // Check if category query param is in URL to sync search select value
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has("category")) {
      searchSelect.value = urlParams.get("category");
    }

    // Direct change event to immediately open shop.html?category=...
    searchSelect.onchange = function() {
      const val = this.value;
      if (val) {
        window.location.href = `shop.html?category=${encodeURIComponent(val)}`;
      } else {
  ...
  // 2. Update Navigation Row "Shop By Categories" Dropdown Menu
  if (navDropdownMenu) {
    let menuItems = `
      <a href="shop.html" style="display: block; padding: 0.65rem 1.25rem; color: var(--color-charcoal); text-decoration: none; font-weight: 700; font-size: 0.85rem; border-bottom: 1px solid rgba(0,0,0,0.08); transition: background 0.15s, color 0.15s;" onmouseover="this.style.background='rgba(115,28,56,0.06)';this.style.color='var(--color-maroon)'" onmouseout="this.style.background='';this.style.color='var(--color-charcoal)'">All Categories</a>
    `;
    ...
    categories.forEach((cat, idx) => {
      ...
      menuItems += `
        <a href="shop.html?category=${encodeURIComponent(cName)}" style="display: block; padding: 0.65rem 1.25rem; color: var(--color-charcoal); text-decoration: none; font-weight: 600; font-size: 0.85rem; ${borderStyle} transition: background 0.15s, color 0.15s;" onmouseover="this.style.background='rgba(115,28,56,0.06)';this.style.color='var(--color-maroon)'" onmouseout="this.style.background='';this.style.color='var(--color-charcoal)'">${cName}</a>
      `;
    });

    navDropdownMenu.innerHTML = menuItems;
  }

  // 3. Update Mega Menu Column "By Category"
  const megaCategoryCol = document.querySelector(".mega-menu-column");
  if (megaCategoryCol) {
    let megaItems = `<h4>By Category</h4>`;
    categories.forEach(cat => {
      const cName = cat.name || cat.title;
      megaItems += `<a href="shop.html?category=${encodeURIComponent(cName)}">${cName}</a>`;
    });
    megaCategoryCol.innerHTML = megaItems;
  }

  // 4. Update Mobile Submenu Links
  if (mobileSubmenu) {
    let mobileItems = `<li><a href="shop.html" style="font-weight:700;">All Categories</a></li>`;
    categories.forEach(cat => {
      const cName = cat.name || cat.title;
      mobileItems += `<li><a href="shop.html?category=${encodeURIComponent(cName)}">${cName}</a></li>`;
    });
    mobileSubmenu.innerHTML = mobileItems;
  }
}
